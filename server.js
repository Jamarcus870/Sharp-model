...
var BOOKMARKLET_JS = `
(function() {
  // Remove any existing Sharp Scanner overlay
  var existing = document.getElementById('sharp-scanner-overlay');
  if (existing) { existing.remove(); return; }

  // Create overlay
  var overlay = document.createElement('div');
  overlay.id = 'sharp-scanner-overlay';
  overlay.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:#060810;z-index:999999;overflow-y:auto;font-family:monospace;padding:16px;';
  
  var header = '<div style="color:#00E5A0;font-size:16px;font-weight:700;letter-spacing:0.2em;margin-bottom:4px;">⚡ SHARP SCANNER</div>';
  var sub = '<div style="color:#4A4E6A;font-size:9px;letter-spacing:0.12em;margin-bottom:16px;">READING PRIZEPICKS LINES...</div>';
  var content = '<div id="ss-content" style="font-size:12px;color:#C8CCE0;"></div>';
  var closeBtn = '<button onclick="document.getElementById(\'sharp-scanner-overlay\').remove()" style="position:fixed;top:16px;right:16px;background:#1E2235;border:none;color:#C8CCE0;padding:6px 12px;border-radius:4px;cursor:pointer;font-family:monospace;font-size:11px;z-index:1000000;">✕ CLOSE</button>';
  
  overlay.innerHTML = header + sub + closeBtn + content;
  document.body.appendChild(overlay);
  
  var statusEl = document.getElementById('ss-content');
  statusEl.innerHTML = '<div style="color:#FFD166;">Fetching live lines...</div>';

  // Fetch PrizePicks API directly (same domain - no CORS)
  ...
  .then(function(ppData) {
    ...
    (ppData.included || []).forEach(function(item) {
      if (item.type === 'new_player') players[item.id] = { name: (item.attributes && item.attributes.name) || 'Unknown', team: (item.attributes && item.attributes.team) || '' };
      if (item.type === 'league') leagues[item.id] = (item.attributes && item.attributes.name) || '';
    });
    var props = [];
    ...
    statusEl.innerHTML = '<div style="color:#FFD166;">Got ' + props.length + ' props — scoring...</div>';
    
    // Send to Sharp server for scoring
  ...
  .then(function(data) {
    ...
    var html = '<div style="color:#00E5A0;font-size:10px;font-weight:700;letter-spacing:0.15em;margin-bottom:12px;">' + edges.length + ' EDGES FOUND — ' + data.statsPlayers + ' PLAYERS TRACKED</div>';
    
    function renderEdges(list, color, label) {
      if (!list.length) return '';
      var h = '<div style="font-size:9px;font-weight:700;color:' + color + ';letter-spacing:0.15em;margin-bottom:8px;margin-top:14px;">' + label + ' (' + list.length + ')</div>';
      list.forEach(function(e) {
        var dirColor = e.direction === 'HIGHER' ? '#00E5A0' : '#FF6B9D';
        h += '<div style="background:#0B0D16;border:1px solid ' + color + '33;border-left:3px solid ' + color + ';border-radius:6px;padding:10px 12px;margin-bottom:8px;">';
        h += '<div style="font-size:13px;font-weight:700;color:#E0E0E8;margin-bottom:2px;">' + e.player + '</div>';
        h += '<div style="font-size:18px;font-weight:700;color:#FF8C42;">' + e.line + ' <span style="font-size:11px;color:#4A4E6A;">' + e.stat + '</span> <span style="font-size:12px;color:' + dirColor + ';">' + e.direction + '</span></div>';
        h += '<div style="font-size:10px;color:' + color + ';margin-top:4px;">Avg ' + e.avg + ' → cushion +' + e.cushion + (e.hitRate ? ' | Hit rate ' + e.hitRate + '%' : '') + '</div>';
        h += '</div>';
      });
      return h;
    }
    
    html += renderEdges(strong, '#00E5A0', '🔒 STRONG LOCKS');
    html += renderEdges(locks, '#00E5A0', 'LOCKS');
    html += renderEdges(leans, '#FFD166', 'LEANS');
  ...
  .catch(function(err) {
    statusEl.innerHTML = '<div style="color:#FF4D6D;">Error: ' + err.message + '</div>';
  });
})();
`;
...
async function startScan() {
  ...
  setStatus("Fetching live props...", "#FFD166");

  try {
    // Step 1: Fetch PrizePicks directly from browser (bypasses IP block)
    setStatus("Pulling PrizePicks lines...", "#FFD166");
    var ppRes = await fetch("https://api.prizepicks.com/projections?per_page=250&single_stat=true", {
      headers: {
        "Accept": "application/json",
    ...
    if (!ppRes.ok) throw new Error("PrizePicks returned " + ppRes.status);
    var ppData = await ppRes.json();
    
    // Parse props from PrizePicks response
    var players = {}, leagues = {};
    (ppData.included || []).forEach(function(item) {
      if (item.type === "new_player") players[item.id] = { name: (item.attributes && item.attributes.name) || "Unknown", team: (item.attributes && item.attributes.team) || "" };
      if (item.type === "league") leagues[item.id] = (item.attributes && item.attributes.name) || "";
    ...
    setStatus("Got " + props.length + " props — scoring edges...", "#FFD166");

    // Step 2: Send props to server for scoring against real game logs
    var scoreRes = await fetch(SERVER + "/score", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ props: props })
    });
    ...
    var data = await scoreRes.json();

    allEdges = data.edges || [];
    var statsPlayers = data.statsPlayers || 0;

    setStatus("✓ " + allEdges.length + " edges found — " + statsPlayers + " players tracked", "#00E5A0");
