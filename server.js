...
function getAvg(player, stat) {
  ...
  // Try stat variations
  const statVariants = {
    // MLB pitcher exact PrizePicks names
    'Pitcher Strikeouts': 'Strikeouts',
    'Strikeouts': 'Strikeouts',
    'Pitcher Fantasy Points': null,
    'Pitcher Fantasy Score': null,
    'Hitter Fantasy Points': null,
    'Pitches Thrown': null,
...
var server = http.createServer(function(req, res) {
  ...
  if (url === "/stats") {
    ...
    cachedProps.forEach(function(p) {
      if (!statNames[p.stat]) statNames[p.stat] = 0;
      statNames[p.stat]++;
    });
    res.writeHead(200);
  ...
  if (url === "/sample") {
    res.writeHead(200);
    res.end(JSON.stringify(cachedProps.slice(0, 10)));
    return;
  }
  res.writeHead(404);
