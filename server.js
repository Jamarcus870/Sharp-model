...
// Format: 'Player Name': { stat: 'Stat Type', avg: number, sport: 'SPORT' }
const DB = {
  // MLB PITCHERS - Strikeouts per start
  'Zack Wheeler':       { stat: 'Strikeouts', avg: 7.2, sport: 'MLB', note: 'Elite May â 1.67 ERA, 30:5 K:BB' },
  'Max Meyer':          { stat: 'Strikeouts', avg: 6.2, sport: 'MLB', note: '5-0, 2.52 ERA, 68 Ks in 60.2 IP' },
  'MacKenzie Gore':     { stat: 'Strikeouts', avg: 5.5, sport: 'MLB', note: '62 Ks on season, short outing risk' },
  'Spencer Arrighetti': { stat: 'Strikeouts', avg: 5.5, sport: 'MLB', note: 'Road K/9 6.89 vs Home 10.18' },
  'Grant Holmes':       { stat: 'Strikeouts', avg: 4.2, sport: 'MLB', note: '4.2 avg â line is above avg' },
  'Jack Flaherty':      { stat: 'Strikeouts', avg: 4.6, sport: 'MLB', note: '5.94 ERA, command breaking down' },
  'Chris Sale':         { stat: 'Strikeouts', avg: 7.8, sport: 'MLB', note: 'Elite 2026 â 1.89 ERA' },
  'Braxton Ashcraft':   { stat: 'Strikeouts', avg: 6.5, sport: 'MLB', note: '3-2, 2.89 ERA, 65 Ks' },
  'Kyle Harrison':      { stat: 'Strikeouts', avg: 6.8, sport: 'MLB', note: 'Strong K upside' },
  'Cristopher Sanchez': { stat: 'Strikeouts', avg: 5.8, sport: 'MLB', note: 'Consistent K producer' },
  'Lucas Giolito':      { stat: 'Strikeouts', avg: 3.0, sport: 'MLB', note: 'Only 1 MLB start in 2026 â 3 Ks' },
  'Chris Paddack':      { stat: 'Strikeouts', avg: 4.5, sport: 'MLB', note: 'Limited 2026 data' },
  // MLB PITCHERS - Walks per start
  'Jack Flaherty_Walks Allowed':   { stat: 'Walks Allowed', avg: 2.9, sport: 'MLB', note: '14% walk rate â career worst' },
  'Grant Holmes_Walks Allowed':    { stat: 'Walks Allowed', avg: 2.3, sport: 'MLB', note: '14 BB in 32.1 IP' },
  'Lucas Giolito_Walks Allowed':   { stat: 'Walks Allowed', avg: 3.5, sport: 'MLB', note: '3 walks in debut, career 3.5 avg' },
  'Spencer Arrighetti_Walks Allowed': { stat: 'Walks Allowed', avg: 2.8, sport: 'MLB', note: '4 straight starts with 3+ walks' },
  // MLB PITCHERS - Outs per start  
  'Grant Holmes_Pitching Outs':    { stat: 'Pitching Outs', avg: 14.4, sport: 'MLB', note: 'Recency weighted 14.4 outs' },
  'Lucas Giolito_Pitching Outs':   { stat: 'Pitching Outs', avg: 15.0, sport: 'MLB', note: '5+ innings in debut' },
  'Colin Rea_Pitching Outs':       { stat: 'Pitching Outs', avg: 14.4, sport: 'MLB', note: '4 of 4 starts under 16.5' },
  // NBA PLAYERS
  'Shai Gilgeous-Alexander': { stat: 'Points', avg: 31.4, sport: 'NBA', note: 'MVP favorite â elite scorer' },
  'Isaiah Hartenstein':      { stat: 'Points+Rebounds+Assists', avg: 21.5, sport: 'NBA', note: '81% hit rate at 21+ min' },
  "De'Aaron Fox":            { stat: 'Rebounds+Assists', avg: 11.1, sport: 'NBA', note: '100% hit rate in WCF (3/3)' },
  'Victor Wembanyama':       { stat: 'Points', avg: 26.4, sport: 'NBA', note: 'Elite scorer, foul trouble risk' },
  'Nikola Jokic':            { stat: 'Points+Rebounds+Assists', avg: 52.0, sport: 'NBA', note: 'Triple double machine' },
  'Luka Doncic':             { stat: 'Points', avg: 29.0, sport: 'NBA', note: 'Elite scorer' },
  'Jayson Tatum':            { stat: 'Points', avg: 26.9, sport: 'NBA', note: 'Celtics star' },
  'Anthony Edwards':         { stat: 'Points', avg: 27.6, sport: 'NBA', note: 'Wolves â explosive scorer' },
  // WNBA PLAYERS
  'Caitlin Clark':        { stat: 'Assists', avg: 9.0, sport: 'WNBA', note: '4 straight games at 9+ assists' },
  'Caitlin Clark_Points': { stat: 'Points', avg: 22.3, sport: 'WNBA', note: 'Elite scorer' },
  'Paige Bueckers':       { stat: 'Points', avg: 22.1, sport: 'WNBA', note: 'Elite scorer vs weak LVA defense' },
  'A\\'ja Wilson':         { stat: 'Points', avg: 27.0, sport: 'WNBA', note: 'Best player in WNBA' },
  'Kelsey Mitchell':      { stat: 'Points', avg: 22.3, sport: 'WNBA', note: '22.3 PPG but GSV matchup kills it' },
  'Sabrina Ionescu':      { stat: 'Points', avg: 18.5, sport: 'WNBA', note: 'Elite scorer and shooter' },
  'Breanna Stewart':      { stat: 'Points', avg: 22.0, sport: 'WNBA', note: 'Liberty star' },
  'Arike Ogunbowale':     { stat: 'Points', avg: 23.1, sport: 'WNBA', note: 'Wings â volume scorer' },
};

// Lookup by player+stat combination
function getAvg(player, stat) {
  // Try exact player+stat combo first
  const comboKey = player + '_' + stat;
  if (DB[comboKey]) return DB[comboKey];
  // Try player alone
  if (DB[player]) {
    const d = DB[player];
    // Match if stat contains the DB stat or vice versa
    if (stat.toLowerCase().includes(d.stat.toLowerCase()) || 
        d.stat.toLowerCase().includes(stat.toLowerCase())) {
      return d;
    }
  }
  return null;
