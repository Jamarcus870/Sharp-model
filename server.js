...
setInterval(fetchUnderdogProps, 25 * 60 * 1000);
      ...
      try {
        ...
        if (json.players) {
          json.players.forEach(function(p) {
            players[p.id] = { name: p.first_name + " " + p.last_name, team: p.team_abbreviation || "" };
          });
        }
        var props = [];
        var lines = json.over_under_lines || json.lines || json.appearances || [];
        lines.forEach(function(line) {
          try {
            var playerId = line.over_under && line.over_under.appearance && line.over_under.appearance.player_id;
            var player = players[playerId] || {};
            var sport = (line.over_under && line.over_under.appearance && line.over_under.appearance.sport_id) || "OTHER";
            sport = sport.toUpperCase();
            if (sport === "BASKETBALL") sport = "NBA";
            if (sport === "BASEBALL") sport = "MLB";
            if (sport === "FOOTBALL") sport = "NFL";
            if (sport === "HOCKEY") sport = "NHL";
            var statName = (line.over_under && line.over_under.title) || line.stat_value || "";
            var lineVal = parseFloat(line.stat_value || line.over_under && line.over_under.line || 0);
            var overMult = parseFloat(line.options && line.options[0] && line.options[0].payout_multiplier || 1);
            var underMult = parseFloat(line.options && line.options[1] && line.options[1].payout_multiplier || 1);
            if (lineVal > 0 && player.name) {
              props.push({
                id: "ud_" + (line.id || Math.random()),
                player: player.name,
                team: player.team || "",
                sport: sport,
                stat: statName,
        ...
        underdogProps = props;
        console.log("Fetched " + props.length + " Underdog props");
      } catch(e) {
        console.error("Underdog parse error: " + e.message);
      }
    });
  });
  req.on("error", function(err) { console.error("Underdog error: " + err.message); });
...
const DB = {
  ...
  // ââ WNBA PLAYERS â POINTS ââ
  'Caitlin Clark_Points':           { avg: 22.3, sport: 'WNBA', note: 'Elite scorer â IND Fever' },
  'Paige Bueckers_Points':          { avg: 22.1, sport: 'WNBA', note: 'Elite scorer â DAL Wings' },
  "A'ja Wilson_Points":             { avg: 27.0, sport: 'WNBA', note: 'Best player in WNBA â LVA' },
  'Kelsey Mitchell_Points':         { avg: 22.3, sport: 'WNBA', note: '22.3 PPG â IND Fever' },
  'Sabrina Ionescu_Points':         { avg: 18.5, sport: 'WNBA', note: 'Elite scorer â NYL' },
  'Breanna Stewart_Points':         { avg: 22.0, sport: 'WNBA', note: 'NYL Liberty star' },
  'Arike Ogunbowale_Points':        { avg: 23.1, sport: 'WNBA', note: 'DAL Wings â volume scorer' },
  'Gabby Williams_Points':          { avg: 16.0, sport: 'WNBA', note: 'GSV â volatile 7-19 range' },
  'Chelsea Gray_Points':            { avg: 9.8,  sport: 'WNBA', note: 'LVA â floor general' },
  'Jackie Young_Points':            { avg: 14.2, sport: 'WNBA', note: 'LVA â secondary scorer' },
  'Jewell Loyd_Points':             { avg: 18.0, sport: 'WNBA', note: 'LVA â elite scorer' },
  'Napheesa Collier_Points':        { avg: 24.1, sport: 'WNBA', note: 'MIN Lynx â elite scorer' },
  'Courtney Williams_Points':       { avg: 14.0, sport: 'WNBA', note: 'CHI Sky' },
  'Aliyah Boston_Points':           { avg: 13.5, sport: 'WNBA', note: 'IND Fever â inside scorer' },
  'Dearica Hamby_Points':           { avg: 12.0, sport: 'WNBA', note: 'LVA â versatile forward' },
  'Kayla Thornton_Points':          { avg: 16.0, sport: 'WNBA', note: 'GSV â post-knee surgery' },

  // ââ WNBA PLAYERS â REBOUNDS ââ
  "A'ja Wilson_Rebounds":           { avg: 11.2, sport: 'WNBA', note: 'Dominant rebounder' },
  'Breanna Stewart_Rebounds':       { avg: 8.5,  sport: 'WNBA', note: 'Elite rebounder' },
  'Napheesa Collier_Rebounds':      { avg: 9.8,  sport: 'WNBA', note: 'Elite rebounder' },
  'Gabby Williams_Rebounds':        { avg: 6.0,  sport: 'WNBA', note: '6.0 RPG but minutes dependent' },
  'Aliyah Boston_Rebounds':         { avg: 8.5,  sport: 'WNBA', note: 'IND â strong rebounder' },
  'Kayla Thornton_Rebounds':        { avg: 5.5,  sport: 'WNBA', note: 'GSV â 5.5 RPG' },
  'Dearica Hamby_Rebounds':         { avg: 7.5,  sport: 'WNBA', note: 'Strong rebounder' },
  'Caitlin Clark_Rebounds':         { avg: 4.4,  sport: 'WNBA', note: 'Guard â moderate rebounding' },

  // ââ WNBA PLAYERS â ASSISTS ââ
  'Caitlin Clark_Assists':          { avg: 9.0,  sport: 'WNBA', note: '4 straight games 9+ assists' },
  'Chelsea Gray_Assists':           { avg: 6.8,  sport: 'WNBA', note: 'Elite playmaker â LVA' },
  'Sabrina Ionescu_Assists':        { avg: 5.5,  sport: 'WNBA', note: 'Elite passer' },
  'Paige Bueckers_Assists':         { avg: 4.5,  sport: 'WNBA', note: 'Good playmaker' },
  'Arike Ogunbowale_Assists':       { avg: 3.5,  sport: 'WNBA', note: 'Scorer first' },

  // ââ WNBA PLAYERS â COMBO STATS ââ
  'Caitlin Clark_Pts+Rebs+Asts':    { avg: 35.7, sport: 'WNBA', note: '22.3+4.4+9.0' },
  "A'ja Wilson_Pts+Rebs+Asts":      { avg: 41.2, sport: 'WNBA', note: '27.0+11.2+3.0' },
  'Caitlin Clark_Pts+Asts':         { avg: 31.3, sport: 'WNBA', note: '22.3+9.0' },
  'Caitlin Clark_Pts+Rebs':         { avg: 26.7, sport: 'WNBA', note: '22.3+4.4' },
  'Caitlin Clark_Rebs+Asts':        { avg: 13.4, sport: 'WNBA', note: '4.4+9.0' },
};
...
function getCushion(avg, line, stat) {
  // Returns { cushion, direction }
  // direction = which side has the edge
  const overCushion = avg - line;   // edge on HIGHER if avg > line
  const underCushion = line - avg;  // edge on LOWER if line > avg
  if (overCushion >= underCushion) {
    return { cushion: overCushion, direction: 'HIGHER' };
  } else {
    return { cushion: underCushion, direction: 'LOWER' };
  }
}
...
function analyzeAndRender() {
  ...
  const scored = allProps.map(p => {
    ...
    const cushionResult = getCushion(dbEntry.avg, p.line, p.stat);
    const cushion = cushionResult.cushion;
    const direction = cushionResult.direction;
...
var server = http.createServer(function(req, res) {
  ...
  if (url === "/props") {
    var allProps = cachedProps.concat(underdogProps);
    res.writeHead(200);
    res.end(JSON.stringify({ props: allProps, count: allProps.length, lastFetch: lastFetch, status: "ok", prizepicks: cachedProps.length, underdog: underdogProps.length }));
    return;
  }
  if (url === "/health") {
    res.writeHead(200);
  ...
  if (url === "/stats") {
    var statNames = {};
    cachedProps.forEach(function(p) {
      var key = p.sport + ":" + p.stat;
      statNames[key] = (statNames[key] || 0) + 1;
    });
    res.writeHead(200);
    res.end(JSON.stringify(statNames));
    return;
  }
